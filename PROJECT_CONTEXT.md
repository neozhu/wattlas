...
- Local candidate snapshot `2026-07-21T06-27-02Z`, which retains 4,247 data centres, 101 water assets, 7,694 mappable industrial-load assets, 27 hydrogen-network context assets, 238,395 power-source records, 129,664 published power plants, 3,229 ADM1 boundaries, 575 cities, and 60,600 grid features. The canonical generator artifacts preserve 77,164 direct GEM Wiki dossier links alongside the preferred primary source URL.
- India is the approved initial country selection and map centre on first load; the global zoom level remains unchanged.
- The compact selected-project card opens external project evidence in a new tab. Generator dossiers prefer an exact GEM Wiki project page, then the preferred public source, then a GEM Wiki name search; other infrastructure uses its published project/source URL when available.
