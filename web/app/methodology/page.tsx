...
async function loadCatalog(): Promise<{ catalog: SourceCatalog; generatedAt: string | null; sourceCoverage: SourceCoverage | null }> {
  ...
  try {
    ...
    if (!relative) return { catalog: { schemaVersion: "1.0", sources: [] }, generatedAt: manifest.generatedAt, sourceCoverage: manifest.sourceCoverage ?? null };
    ...
    return { catalog: sourceCatalogSchema.parse(JSON.parse(await readFile(file, "utf8"))), generatedAt: manifest.generatedAt, sourceCoverage: manifest.sourceCoverage ?? null };
  } catch {
    return { catalog: { schemaVersion: "1.0", sources: [] }, generatedAt: null, sourceCoverage: null };
  }
}
